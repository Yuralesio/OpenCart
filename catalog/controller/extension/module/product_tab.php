...
class ControllerExtensionModuleProductTab extends Controller {
	public function index($setting) {
			...
			if ($latest_results) {
				foreach ($latest_results as $result) {
					...
					$data['latest_products'][] = array(
						...
						'thumb'       => $image,
						'name'        => $result['name'],
						'description' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, 65) . '..',
						'price'       => $price,
						'special'     => $special,
						'off_label'		  => $off_label,
						'tax'         => $tax,
			...
			if ($special_results) {
				foreach ($special_results as $result) {
					...
					$data['special_products'][] = array(
						...
						'href'        => $this->url->link('product/product', 'product_id=' . $result['product_id']),
						'quickview_href'        => $this->url->link('product/quickview', 'product_id=' . $result['product_id']),
					); //echo "<pre>";print_r($data['special_products']);exit();
				}
			}
		}
			...
			foreach ($products as $product_id) {
				...
				if ($product_info) {
					...
					$data['featured_products'][] = array(
						...
						'description' => utf8_substr(strip_tags(html_entity_decode($product_info['description'], ENT_QUOTES, 'UTF-8')), 0, 65) . '..',
						'price'       => $price,
						'special'     => $special,
						'off_label'		  => $off_label,
						'tax'         => $tax,
						...
						'quickview_href'        => $this->url->link('product/quickview', 'product_id=' . $product_info['product_id']),
					);
				}
			}
		}
		...
		$data['module'] = $module++;
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_theme') . '/template/extension/module/product_tab.twig')) {
